...
async def add_deleter(original_message: discord.Message,
    ...
    first_message = sent_messages.pop(0)
    await first_message.add_reaction(DELETE_REACTION_EMOJI)
    first_embed = first_message.embeds[0]
    first_embed.set_author(
        name=getattr(first_embed.author, 'name', EmptyEmbed),
        icon_url=getattr(first_embed.author, 'icon_url', EmptyEmbed),
